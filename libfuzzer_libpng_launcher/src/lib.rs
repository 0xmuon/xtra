...
use libafl::{
    ...
    stages::{
        ...
        tmin::{ObserverEqualityFactory, StdTMinMutationalStage},
        time_tracker::TimeTrackingStageWrapper, TrimTime,
    },
...
pub extern "C" fn libafl_main() {
            ...
            + SendExiting,
    {
        ...
        // Add a minimization stage so `AflStatsStage` can update `trim_time`.
        let tmin_factory = ObserverEqualityFactory::new(&edges_observer);
        let tmin_mutator = HavocScheduledMutator::new(havoc_mutations());
        let tmin_stage = StdTMinMutationalStage::new(tmin_mutator, tmin_factory, 128);
        let tmin_stage = TimeTrackingStageWrapper::<TrimTime, _, _>::new(tmin_stage);
